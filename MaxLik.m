...
try
  switch lower(solver)
    ...
    case 'particleswarm'
      Objective = @(P) -sum(loglikfun(ParamsTransformInv(SelectParams(P')),obs,varargin{:}),1)'/nobs;
      solveroptions.InitialSwarm = PARAMS';
      problem = struct('solver'   , solver,...
                       'objective', Objective,...
    ...
    case 'ga'
      Objective = @(P) -sum(loglikfun(ParamsTransformInv(SelectParams(P')),obs,varargin{:}),1)'/nobs;
      solveroptions.InitialPopulation = PARAMS';
      problem = struct('solver'    , solver,...
    ...
    case 'pswarm'
      %% PSwarm
      if strcmpi(options.Vectorized,'on'), solveroptions.Vectorized = 1; end
